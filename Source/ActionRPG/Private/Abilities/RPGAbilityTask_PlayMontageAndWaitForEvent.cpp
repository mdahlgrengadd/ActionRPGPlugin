...
{
	if (IsValid(Ability.Get()) && Ability->GetCurrentMontage() == MontageToPlay)
	{
...
{
	//if (Ability == nullptr)
	if (!IsValid(Ability.Get()))
	{
		UE_LOG(LogTemp, Error, TEXT("URPGAbilityTask_PlayMontageAndWaitForEvent::Activate: Ability is null"));
		return;
	}
